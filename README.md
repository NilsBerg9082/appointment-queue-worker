...
This is the small Python service I wanted when moving a queue-backed workflow out of a Next.js request handler. Infrai keeps the queue calls behind one API and a single `INFRAI_API_KEY`; the worker can stay focused on concurrency, pacing, and the appointment decision.
